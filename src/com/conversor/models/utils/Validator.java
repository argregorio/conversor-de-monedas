...
public class Validator {
    ...
    public int validateTargetInputMintoMax(Scanner input) {
        while (this.targetIndex > this.maxIndex || this.targetIndex < this.minIndex) {
            System.out.println("Opción inválida. Por favor, intenta con una de la lista: ");
            int userIn = input.nextInt();
    ...
    public int validateBaseTargetIndexNotEquals(Scanner input) {
        while (this.baseIndex == this.targetIndex) {
            System.out.println("La moneda de origen y la moneda a la cual deseas convertir son iguales");
            System.out.println("Ingresa una moneda de cambio distinta: ");
            int userIn = input.nextInt();
            setTargetIndex(userIn);
        }
    ...
    public int validateUserDecision(Scanner input, int optionContext) {
        while (!this.decision.equalsIgnoreCase("S") && !this.decision.equalsIgnoreCase("N")) {
            System.out.println("Opcion invalida. Por favor digita 'S' para continuar o 'N' para regresar al menu principal: ");
    ...
    public Country validateAddItemtoList(Country country, List<Country> countryList, List<Country> compareList, Scanner input) {
        ...
        while (localCountryCodes.contains(countryCode)) {
            System.out.println("La moneda que ingresaste ya se encuentra en la lista, por favor intenta ingresando uno diferente: ");
            int userIn = input.nextInt();
    ...
    public boolean validateArraySize(List<Country> countries) {
        if (countries.size() == 2) {
            System.out.println("No puedes eliminar más monedas de la lista, se necesitan al menos dos para realizar las conversiones.");
            return false;
        } else {
            return true;
        }
